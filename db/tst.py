...
from util import StockCurrentInfo
...
if '__main__' == __name__:
    ses = get_session()
    sina = easyquotation.use('qq')

    objs = []
    all_stock = sina.all
    for code, st in all_stock.items():
        # print(code, st)
        objs.append(StockCurrentInfo(code=code, name=st['name'],
                                    high=st['high'], low=st['low'],
                                    ltsz=st['流通市值'], total_sz=st['总市值'],
                                    turnover=st['turnover'], volume=st['成交量(手)'],
                                    amount=st['成交额(万)'], pb=st['PB'], pe=st['PE'],
                                    date=date.today()))

    ses.bulk_save_objects(objs)
